...
from .pskd_trainer import PSKDITrainer

def get_trainer(net, train_loader: DataLoader, val_loader: DataLoader, ood_loader: DataLoader,
                config: Config):
    if type(train_loader) is DataLoader:
        ...
        if config.trainer.name in ['cider', 'npos', 'pskd_i']:
            return trainers[config.trainer.name](net, train_loader, val_loader, ood_loader,
                                                 config)
        else:
